...
let make_info_list answer =
  BatList.mapi
    (fun pos c -> { aletter = c; checked = false; pos })
    (make_list answer)
...
let test_answer answer = print_answer (make_info_list answer)

let check_dupe_position c pos string guess_pos =
  let correct_position = BatString.find_from c pos string in
...
let rec check_letters info_list answer c pos guess =
  ...
  | h :: t ->
      ...
      if h.aletter = c && h.checked = false then
        let () =
          print_string
            ("h.aletter: " ^ String.make 1 h.aletter ^ " char: "
           ^ String.make 1 c ^ "\n")
        in
        check_position c h.pos guess
      else if h.aletter = c && h.checked = true then
...
let assign_feedback answer c pos guess =
  if String.contains answer c = false then
    ...
    let answer_info = make_info_list answer in
